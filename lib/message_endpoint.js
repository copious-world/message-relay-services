...
class Communicator extends EventEmitter {
     ...
     * 
     * Note: publication has been simplified to a single case, which is always on a path. 
     * This pathless version of app publication is kept, but sets the path to the topic. 
     * A subscriber must subscribe to the topic on a path equal to the topic to receive from this method.
     * The method `app_publish_on_path` does not have this restriction.
     * 
     * @param {string} topic 
     * @param {object} msg_obj 
     */
...
class Server extends Communicator {
    ...
    closeAll(cb,force) {
        if ( force ) {
            if ( Object.keys(this.messenger_connections).length > 0 ) {
                for ( let mcon of Object.entries(this.messenger_connections) ) {
                    if ( mcon.writer && (typeof mcon.writer.close === 'function') ) {
                        mcon.writer.close()
                    }
                }
            }    
