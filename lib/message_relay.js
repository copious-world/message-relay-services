...
class JsonMessageHandler extends JSONMessageQueue {
    ...
    async dequeue_and_forward() {
        ...
        while ( this.message_queue.length !== 0 )  {
            ...
            if ( path ) {
                let shared_path_handler = this.message_paths[path]   // don't forget this is the path handler instance
...
class Communicator {
    ...
    constructor(conf,fanoutRelayer) {
        ...
        for ( let a_path in path_types ) {
            ...
            this.message_paths[a_path] = mpath  // mpath is the class instance (default is PathHandler)
        }
    ...
    add_connection(client_name,writer) {
        //
        if ( NOISY ) console.log(`message relay server :: new client connected: ${client_name}`);
        //
