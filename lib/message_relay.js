                            ...
                            if (  (this.current_message._ps_op !== undefined ) && (this.current_message._ps_op !== 'pub') ) {
                                ...
                                if (  this.current_message._ps_op === 'sub'  ) {   // sub from pub/sub on this path.
                                    ...
                                    if ( topic ) {
                                        ...
                                        shared_path_handler.subscribe(topic,this.current_message,listener)
                                        result = "OK"
                                    }
                                    ...
                                    if ( topic ) {
                                        ...
                                        shared_path_handler.unsubscribe(topic,listener)   // remove this socket's handler
                                        delete topics[topic]
                                        result = "OK"    
                                    }
                                }
    ...
    cleanup() {
        ...
        if ( shared_path_handler ) {   // does nothing to the path handler except remove the listener
            ...
            for ( let topic in topics ) {
                let listener = topics[topic]
                shared_path_handler.unsubscribe(topic,listener)
                delete topics[topic]
            }
...
class Communicator {
    ...
    add_message_handler(writer,client_name) {
        //
...
class Server extends Communicator {
    ...
    onClientConnected_func(sock) {
        ...
        let client_name = this.add_message_handler(sock,client_name)
        //
        // 1. data
        sock.on('data',((com) => { return (data) => {
            com.add_data(client_name,data)
        }})(this))
        //
        //  2. close
