...
class Client extends EventEmitter {
        ...
        client.socket.on('data', (data) => {
            ...
            if ( mqueue.message_queue.length ) {
                ...
                if ( message._response_id !== undefined ) {
                    ...
                    client.handle_unsolicited(message)
                }
            }
        });
        //
    ...
    //
    handle_unsolicited(message) {
        if ( message !== undefined ) {
            let topic = message.topic
            this.emit(`update-${topic}`,message)
        } 
    }
    ...
    async subscribe(topic,message,handler) {
        if ( handler !== undefined && (typeof handler === "function") ) {
            ...
            this.subcriptions[`update-${topic}`] = handler
        }
        message._ps_op = "sub"
        message.topic = topic
        return await this.sendMessage(message)
    ...
    unsubscribe(topic) {
        ...
        this.removeListener(`update-${topic}`,handler)
        let message = {
            "_ps_op" : "unsub",
            "topic" : topic
        }
