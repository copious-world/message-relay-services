...
class Client extends EventEmitter {
    ...
    constructor(conf,wrapper) {
        ...
        //
        this.shunt_file = '/message_relay.txt'
        //
    ...
    connection_processing(client,conf) {
        ...
        let socket_ready = () => {
            ...
            if ( client.files_going ) {  // then shunting had to be set to true.. file_only has to be false
        ...
        if ( this.use_tls === false  ) {
            ...
            client.socket.connect(client.port, client.address,socket_ready);    
        } else {
            //
            const tls_options = {
            ...
            client.socket = tls.connect(client.port, client.address, tls_options, () => {
        ...
        client.socket.on('data', (data) => {
            ...
            if ( mqueue.message_queue.length ) {
                ...
                if ( message._response_id !== undefined ) {
                    ...
                    client._handle_unsolicited(message)
                }
            }
        });
        //
    ...
    unsubscribe(topic,path) {
        let handler = this.subcriptions[`update-${topic}`]
