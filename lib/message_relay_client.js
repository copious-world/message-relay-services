...
class Client extends EventEmitter {
    ...
    constructor(conf,wrapper) {
        ...
        //
        this.subcriptions = {}
        this.messages = new JSONMessageQueue()
        //
    ...
    _connect() {
        ...
        if ( this.use_tls === false  ) {
            ...
            this.socket.connect(client.port, client.address,this._connection_handler);    
        } else {
            //
            // ENCRYPTED TLS
            const tls_options = {
                // Necessary only if the server requires client certificate authentication.
                key: fs.readFileSync(this.tls_conf.clien_key),
                cert: fs.readFileSync(this.tls_conf.clien_cert),
            ...
            this.socket = tls.connect(client.port, client.address, tls_options, () => {
    ...
    _setup_connection_handlers(client,conf) {
        //
        client.resp_vector.setup_response_vector()
        // HANDLERS
        client.socket.on('close', (onErr) => {
            ...
            if ( client.attempt_reconnect ) {
                client._attempt_reconnet(conf)
            }
        })
        //
        client.socket.on('data',this._handle_message_data);
        //
        client.socket.on('error',(err) => {
            ...
            if ( client.file_shunting ) {
                client._start_file_shunting(conf)
            }
        })
        //
    }
    ...
    async _shutdown_files_going() {
        if ( this.send_on_reconnect ) {
            this.file_output.close()
            let messages = await fsPromise.readFile(this.shunt_file)
    ...
    _send_to_counted_stream(string) {
        this.message_count++
    ...
    _sendMessage_to_file(message,resolve) {
        let flat_message = JSON.stringify(message,resolve)
        if ( this.file_per_message ) {
            this.file_count++
            ...
            this._send_to_counted_stream(flat_message)
        }
    }

    //
    _message_and_response(resolve,reject) {
    ...
    sendMessage(message) {   // secondary queuing is possible
        return new Promise((resolve, reject) => {
            if ( this.files_only && this.files_going && (this.file_output !== undefined) ) {
                this._sendMessage_to_file(message,resolve)
                ...
                if ( this.files_going && (this.file_output !== undefined) ) {       // default to this when connections fail..
                    this._send_to_counted_stream(JSON.stringify(message))
                    resolve("OK")  // can't ask the file to deliver a response
                } else {
                    this._message_and_response(resolve,reject)
                }
            }
        });
    }
